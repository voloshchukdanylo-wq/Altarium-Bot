...
REGISTRATION_COMPONENTS_V2_LAYOUTS_ENABLED = os.getenv(
    "ENABLE_DISCORD_COMPONENTS_V2_LAYOUTS", ""
).strip().lower() in {
    "1",
    "true",
    "yes",
...
REGISTRATION_COMPONENTS_V2_RUNTIME_ENABLED = all((
    ...
    REGISTRATION_COMPONENTS_V2_LAYOUTS_ENABLED,
))
    ...
    if item.get("item_kind") == "tech":
        lines.extend(
            [
                ...
                f"-# **Оценка техники:** {int(item.get('tech_rating', 1) or 1)}/100",
            ]
        )
    if description:
    ...
    for suffix, mult in units.items():
        if raw.endswith(suffix):
            ...
            if value <= 0:
                raise ValueError("Значение должно быть больше 0")
            return value * mult
    raise ValueError("Формат времени: 24ч / 30м / 10с / 1д (можно: секунды, минуты, часы, дни)")
...
def format_rp_year_embed(
    ...
    em = Embed(
        ...
        description=(
            ...
            f"**Смена поры года:** <t:{int(investments.setdefault('rp_year', {}).get('next_tick_at', int(time.time())))}:R>\n\n"
            f"{mood}"
        ),
        color=color,
    )
...
def archive_country_stats_for_user(user_id: str, reason: str = "unregister"):
    ...
    for season_key, reg in list(registrations.items()):
        ...
        snapshot = {
            ...
            "season": season_key,
            "source_user_id": uid,
            "balances": make_json_snapshot(balances.get(uid, {})),
        ...
        has_data = any(bool(snapshot.get(k)) for k in (
            ...
            "season_progress", "player_state", "investments", "active_investments", "companies",
        )) or relation_snapshot_has_data(snapshot.get("relations", {}))
        if has_data:
            archived[country_archive_key(country_name, season_key)] = snapshot
...
def apply_archived_country_stats(user_id: str, snapshot: dict):
    ...
    balances[uid] = make_json_snapshot(snapshot.get("balances", {})) or {"наличка": 0, "банк": 0, "заморожено": 0, "коины": 0}
    ...
    save_passive_flows(); save_seasons_data(); save_player_state(); save_investments(); save_companies_data(); save_relations_data()
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.UserInputError) and ctx.command is not None:
        ...
        examples = {
            ...
            "список-тикетов": "!список-тикетов",
            "категория-тикетов": "!категория-тикетов <категория>",
            ...
            "вердикты": "!вердикты",
            "принятьверд": "!принятьверд 12",
            "очиститьверды": "!очиститьверды",
...
async def хелп(ctx):
    ...
    command_briefs = {
        ...
        "ролл": "Случайное число, диапазон, несколько бросков или выбор одного ответа из списка.",
        ...
        "экономика": "Настройка ролей уровней экономики по количеству прокачанных сфер.",
        ...
        "вердиктканал": "Отправляет панель, через которую игроки запрашивают вердикт.",
        "вердзаявкиканал": "Назначает канал, куда приходят заявки на вердикт для рассмотрения.",
        "итогвердиктканал": "Назначает канал публикации итогов по вердиктам.",
        "вердикты": "Служебная команда-подсказка для настройки доступа к вердиктам через !разрешить.",
        "принятьверд": "Повторно создаёт меню рассмотрения заявки на вердикт по номеру, если его нужно восстановить вручную.",
        "очиститьверды": "Удаляет все нерассмотренные заявки на вердикт, чтобы игроки могли подать новые.",
        ...
        "вайпроли": "Настройка ролей, которые снимаются при вайпе, и исключений из снятия.",
        ...
        "репутация": "Установить репутацию игрока в процентах (0-100%).",
    ...
    def command_full_text(cmd_name: str) -> str:
        ...
        for cat, names in categories.items():
            if cmd_name in names:
                related = [n for n in sorted(names, key=lambda x: x.casefold()) if n != cmd_name]
                break
    ...
    def build_category_embed(cat_name: str, command_page: int = 0):
        ...
        for cmd_name in shown:
            brief = command_briefs.get(cmd_name, f"Подробная инструкция по команде `!{cmd_name}`.")
            lines.append(f"• `!{cmd_name}` — {brief}")
        embed = Embed(title=f"📘 Хелп — {cat_name}", color=0x3498DB)
        embed.description = (
            f"**Что делает раздел:**\n{category_purpose.get(cat_name, 'Описание не задано.')}\n\n"
            "Выберите конкретную команду в списке ниже, чтобы увидеть полную инструкцию, варианты использования и доступы.\n\n"
    ...
    class HelpCategorySelect(Select):
        ...
        async def callback(self, interaction: Interaction):
            ...
            await interaction.response.edit_message(embed=build_category_embed(self.parent_view.category, 0), view=self.parent_view)

    class HelpCommandSelect(Select):
        def __init__(self, parent_view):
            self.parent_view = parent_view
            commands_list = sorted(categories.get(parent_view.category, []), key=lambda x: x.casefold())
            shown = commands_list[parent_view.command_page * 25 : (parent_view.command_page + 1) * 25]
            options = [
                discord.SelectOption(
                    label=f"!{name}"[:100],
                    value=name,
                    description=(command_briefs.get(name, "Открыть инструкцию по команде")[:100]),
                )
                for name in shown
            ] or [discord.SelectOption(label="Нет команд", value="__none__", description="В этой категории нет команд")]
            super().__init__(placeholder="Выберите команду для полной инструкции...", min_values=1, max_values=1, options=options)
        ...
        async def callback(self, interaction: Interaction):
            ...
            self.parent_view.instruction_page = 0
            self.parent_view.refresh_items()
            await interaction.response.edit_message(embed=build_command_embed(self.values[0], 0), view=self.parent_view)
    ...
    class HelpView(View):
        ...
        def prev_list_button(self):
            view = self
            class Prev(Button):
                def __init__(self): super().__init__(label="◀ Список", style=ButtonStyle.secondary)
                async def callback(self, interaction: Interaction):
                    view.command_page = max(0, view.command_page - 1)
                    view.selected_command = None
                    view.refresh_items()
                    await interaction.response.edit_message(embed=build_category_embed(view.category, view.command_page), view=view)
            return Prev()
        ...
        def prev_instruction_button(self):
            ...
            class PrevInstruction(Button):
                def __init__(self): super().__init__(label="◀ Инструкция", style=ButtonStyle.primary)
                async def callback(self, interaction: Interaction):
                    view.instruction_page = max(0, view.instruction_page - 1)
                    view.refresh_items()
                    await interaction.response.edit_message(embed=build_command_embed(view.selected_command, view.instruction_page), view=view)
        ...
        def back_button(self):
            view = self
            class Back(Button):
                def __init__(self): super().__init__(label="Назад к списку", style=ButtonStyle.success)
                async def callback(self, interaction: Interaction):
                    view.selected_command = None
    ...
    view = HelpView()
    await ctx.send(embed=build_category_embed(view.category, view.command_page), view=view)
    ...
    class PlayerMenuSelect(Select):
        def __init__(self):
            options = [
                ...
                SelectOption(
                    label="Отношения с странами",
                    value="relations",
                    emoji="🌐",
                    description="Мировые отношения и изменение раз в RP-год",
                ),
            ]
            super().__init__(
...
def get_spheres_until_next_economy_level(user_id: str) -> str:
    ...
    for level_text, config in seasons_data.setdefault("economy_levels", {}).items():
        ...
        for value in progress.values():
            try:
                if int(value or 0) >= required_level:
                    ready += 1
            except (TypeError, ValueError):
                pass
        missing = max(0, required_count - ready)
        best_missing = missing if best_missing is None else min(best_missing, missing)
    return "*" if best_missing == 0 else str(best_missing if best_missing is not None else "—")
...
async def панель(ctx):
        ...
        async def callback(self, interaction: Interaction):
            ...
            if choice == "tech_types":
                class TechTypeModal(Modal):
                    ...
                    async def on_submit(self, mi: Interaction):
                        try: price = max(1, int(str(self.price.value).replace('.', '').replace(' ', '')))
                        except Exception:
                            await mi.response.send_message("❌ Цена должна быть числом.", ephemeral=True); return
                        name = str(self.name.value).strip(); items_data.setdefault("tech_types", {})[name] = {"min_price": price}; save_items(); await mi.response.send_message(f"✅ Тип техники **{name}** сохранён: {fmt_money(price)}.", ephemeral=True)
                ...
                def build_tech_types_embed(page: int = 0, selected: str | None = None):
                    ...
                    lines = [f"• **{name}** — минимум {fmt_money(int(cfg.get('min_price', 0) or 0))}" for name, cfg in page_items]
                    ...
                    if selected and selected in items_data.setdefault("tech_types", {}):
                        ...
                        e.add_field(name="Выбранный тип", value=f"**{selected}**\nМинимальная цена: {fmt_money(int(cfg.get('min_price', 0) or 0))}", inline=False)
                ...
                class PanelTechTypesView(View):
                    def __init__(self, page: int = 0, selected: str | None = None):
                        ...
                        add=Button(label="Создать тип техники", style=ButtonStyle.success)
            ...
            if choice == "channels":
                ...
                kind_select = Select(placeholder="Что настраиваем?", options=[
                    ...
                    SelectOption(label="Логи экономики", value="economy_log_channel"),
                    SelectOption(label="Логи сообщений", value="message_log_channel"),
                    SelectOption(label="Занятые страны", value="occupied_registrations_channel"),
                    SelectOption(label="Заявки сфер", value="sphere_requests.channel_id"),
                    SelectOption(label="Итоги сфер", value="sphere_requests.result_channel_id"),
                    SelectOption(label="Панель вердиктов", value="verdicts.panel_channel"),
                    SelectOption(label="Заявки вердиктов", value="verdicts.requests_channel"),
                    SelectOption(label="Итоги вердиктов", value="verdicts.result_channel"),
                    SelectOption(label="Панель партнерств", value="partnership.panel_channel"),
                    SelectOption(label="Заявки партнерств", value="partnership.requests_channel"),
                    SelectOption(label="Публикации партнерств", value="partnership.result_channel"),
                    SelectOption(label="Канал PR-менеджера", value="partnership.manager_post_channel"),
                    SelectOption(label="Панель инвестиций", value="investments.panel_channel"),
                    SelectOption(label="Заявки инвестиций", value="investments.requests_channel"),
                    SelectOption(label="Итоги инвестиций", value="investments.result_channel"),
                    SelectOption(label="Заявки компаний", value="companies.requests_channel"),
                    SelectOption(label="Итоги компаний", value="companies.result_channel"),
                    SelectOption(label="Канал отношений", value="relations.channel_id"),
                ])
    ...
    await ctx.send(embed=build_panel_embed(), view=panel_view)
    ...
    if not ok:
        await ctx.send(
            embed=Embed(
                ...
                description=f"{ctx.author.mention}, вы сможете снова поработать в {cooldown_ready_time(int(time.time()), cooldown_left)}.",
                color=0xF1C40F,
            )
        )
        return
...
async def новостиканал(ctx, channel: discord.TextChannel):
    ids = get_news_channel_ids()
    ids.add(int(channel.id))
    save_news_channel_ids(ids)
    await ctx.send(
        embed=Embed(
            title="✅ Новостной канал добавлен",
            description=f"Канал новостей: {channel.mention}\nВсего каналов: **{len(ids)}**.",
            color=0x00FF00,
        )
    )


@bot.command(name="новостиудалитьканал")
@admin_or_command_access()
async def новостиудалитьканал(ctx, channel: discord.TextChannel):
    ids = get_news_channel_ids()
    if int(channel.id) not in ids:
        await ctx.send(embed=Embed(title="ℹ️ Канал не найден", description=f"{channel.mention} не был новостным каналом.", color=0x3498DB))
        return
    ids.discard(int(channel.id))
    save_news_channel_ids(ids)
    await ctx.send(embed=Embed(title="✅ Новостной канал удалён", description=f"Канал новостей удалён: {channel.mention}", color=0x00FF00))
...
def rename_season_everywhere(old_name: str, new_name: str) -> int:
    ...
    seasons = seasons_data.setdefault("seasons", {})
    if old_key in seasons:
        seasons[new_key] = seasons.pop(old_key)
        changed += 1

    if str(seasons_data.get("active_season") or "") == old_key:
        seasons_data["active_season"] = new_key
        changed += 1

    progress = seasons_data.setdefault("season_user_progress", {})
    if old_key in progress:
        old_progress = progress.pop(old_key)
        if new_key in progress and isinstance(progress[new_key], dict) and isinstance(old_progress, dict):
            merged = dict(progress[new_key])
            merged.update(old_progress)
            progress[new_key] = merged
        else:
            progress[new_key] = old_progress
        changed += 1

    for sphere in seasons_data.setdefault("spheres", {}).values():
        if isinstance(sphere, dict) and str(sphere.get("season") or "") == old_key:
            sphere["season"] = new_key
            changed += 1

    for record in country_stats.values():
        if not isinstance(record, dict):
            continue
        seasons_map = record.get("seasons")
        if isinstance(seasons_map, dict) and old_key in seasons_map:
            old_population = seasons_map.pop(old_key)
            seasons_map.setdefault(new_key, old_population)
            changed += 1

    season_country_to_user, season_user_to_country = ensure_registration_maps()
    for mapping_group in (season_country_to_user, season_user_to_country):
        if old_key in mapping_group:
            old_mapping = mapping_group.pop(old_key)
            if new_key in mapping_group and isinstance(mapping_group[new_key], dict) and isinstance(old_mapping, dict):
                merged = dict(mapping_group[new_key])
                merged.update(old_mapping)
                mapping_group[new_key] = merged
            else:
                mapping_group[new_key] = old_mapping
            changed += 1
    ...
    for profile in country_owners.setdefault("user_profiles", {}).values():
        ...
        if isinstance(registrations, dict) and old_key in registrations:
            reg_record = registrations.pop(old_key)
            if isinstance(reg_record, dict):
                reg_record["season"] = new_key
            registrations[new_key] = reg_record
            changed += 1
        if str(profile.get("active_season") or "") == old_key:
            profile["active_season"] = new_key
            changed += 1

    if str(seasons_data.get("active_season") or "") == new_key:
        country_owners["country_to_user"] = dict(season_country_to_user.get(new_key, {}))
        country_owners["user_to_country"] = dict(season_user_to_country.get(new_key, {}))
...
def get_economy_level_config(level: int) -> dict:
    levels = seasons_data.setdefault("economy_levels", {})
    config = levels.setdefault(str(level), {"sphere_count": 0, "sphere_level": 0, "roles": []})
    config.setdefault("sphere_count", 0)
    config.setdefault("sphere_level", 0)
    config.setdefault("roles", [])
    return config


def parse_role_ids_from_text(guild: discord.Guild, raw: str) -> tuple[list[str], list[str]]:
    role_ids: list[str] = []
    unknown: list[str] = []
    for token in re.split(r"[\s,;]+", str(raw or "").strip()):
        ...
        role_id = match.group(1) if match else token
    ...
    return role_ids, unknown


def get_user_economy_level(user_id: str, season_name: str | None = None) -> tuple[int, dict | None]:
    progress = get_user_progress_for_season(str(user_id), season_name)
    levels = seasons_data.setdefault("economy_levels", {})
    best_level = 0
    best_config = None
    for level_text, config in levels.items():
        try:
            level = int(level_text)
            sphere_count = int(config.get("sphere_count", 0))
            sphere_level = int(config.get("sphere_level", 0))
        except (TypeError, ValueError):
            continue
        if level < 1 or level > 5 or sphere_count <= 0 or sphere_level <= 0:
            continue
        matched_count = 0
        for value in progress.values():
            try:
                current_level = int(value or 0)
            except (TypeError, ValueError):
                current_level = 0
            if current_level >= sphere_level:
                matched_count += 1
        if matched_count >= sphere_count and level > best_level:
            best_level = level
            best_config = config
    return best_level, best_config


async def sync_member_economy_roles(member: discord.Member) -> tuple[int, list[discord.Role], list[discord.Role]]:
    target_level, target_config = get_user_economy_level(str(member.id), seasons_data.get("active_season"))
    configured_role_ids = {
        str(role_id)
        for config in seasons_data.setdefault("economy_levels", {}).values()
        for role_id in config.get("roles", [])
    }
    target_role_ids = {
        str(role_id) for role_id in (target_config or {}).get("roles", [])
    }
    to_remove = [
        role for role in member.roles
        if str(role.id) in configured_role_ids and str(role.id) not in target_role_ids
    ]
    to_add = [
        role for role_id in target_role_ids
        if str(role_id).isdigit()
        if (role := member.guild.get_role(int(role_id))) and role not in member.roles
    ]
    reason = f"Автосинхронизация уровня экономики: {target_level or 'нет уровня'}"
    if to_remove:
        await member.remove_roles(*to_remove, reason=reason)
    if to_add:
        await member.add_roles(*to_add, reason=reason)
    return target_level, to_add, to_remove


async def sync_guild_economy_roles(guild: discord.Guild) -> dict[str, int]:
    """Синхронизирует роли экономики для всех игроков сервера."""
    stats = {"checked": 0, "changed": 0, "added": 0, "removed": 0, "failed": 0}
    for member in guild.members:
        if member.bot:
            continue
        stats["checked"] += 1
        try:
            _, roles_added, roles_removed = await sync_member_economy_roles(member)
        except (discord.Forbidden, discord.HTTPException):
            stats["failed"] += 1
            continue
        added_count = len(roles_added)
        removed_count = len(roles_removed)
...
@bot.command(name="редактсезон")
@admin_or_command_access()
async def редактсезон(ctx, старое_название: str, *, новое_название: str):
    old_name = str(старое_название or "").strip()
    new_name = str(новое_название or "").strip()
    seasons = seasons_data.setdefault("seasons", {})

    if not old_name or old_name not in seasons:
        await ctx.send(
            embed=Embed(
                title="❌ Ошибка",
                description="Сезон не найден. Используйте `!списоксезонов` для просмотра доступных сезонов.",
    ...
    if not new_name:
        await ctx.send(
            embed=Embed(
                title="❌ Ошибка",
                description="Новое название сезона не может быть пустым.",
                color=0xFF0000,
            )
        )
        return

    if new_name in seasons and new_name != old_name:
        await ctx.send(
            embed=Embed(
                title="❌ Ошибка",
                description=f"Сезон **{new_name}** уже существует.",
                color=0xFF0000,
            )
        )
        return

    changed = rename_season_everywhere(old_name, new_name)
    save_seasons_data()
    ...
    await ctx.send(
        embed=Embed(
            title="✅ Сезон обновлён",
            description=f"Сезон **{old_name}** переименован в **{new_name}**. Обновлено связанных записей: **{changed}**.",
            color=0x00FF00,
        )
    )
    await ensure_occupied_registrations_message()
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        async def ensure_pending_request_visible(req_obj: dict):
            ...
            rebuilt = Embed(
                ...
                description=(
                    ...
                    f"\n**Предусмотрена награда:** {', '.join(f'<@&{rid}>' for rid in ((sphere_old or {}).get('levels', [{}])[int(req_obj.get('level', 1)) - 1].get('rewards', []) if int(req_obj.get('level', 1)) - 1 < len((sphere_old or {}).get('levels', [])) else [])) or 'нет'}"
                ),
                color=0x3498DB,
            )
        ...
        embed = Embed(
            ...
            description=(
                ...
                f"\n**Предусмотрена награда:** {', '.join(f'<@&{rid}>' for rid in sphere['levels'][self.level - 1].get('rewards', [])) or 'нет'}"
            ),
            color=0x3498DB,
        )
...
async def approve_sphere_request(interaction: Interaction, request_id: str, note: str | None = None):
    ...
    if get_available_cash(user) < price:
        await interaction.followup.send("Недостаточно доступных средств у игрока.", ephemeral=True)
        return
    ...
    await log_economy_change(
        interaction.guild,
        req["user_id"],
        f"Покупка сферы «{sphere.get('name', 'Без названия')}» уровня {req['level']}",
        cash_delta=-price,
    )
    ...
    req["processed_by"] = interaction.user.id
    if note:
        req["approval_note"] = str(note)
    increment_admin_counter(interaction.user.id, "sphere_approved")
    save_sphere_requests()
    ...
    if result_channel and member:
        ...
        if note:
            description += f"\n\n**Оговорка/замечания к посту:**\n{note}"
        await result_channel.send(embed=Embed(title="✅ ОДОБРЕНО", description=description, color=0x00FF00))
    ...
    status = f"**Одобрено с оговоркой**\nКуратор: {interaction.user.mention}\nЗамечания: {note}" if note else f"**Одобрено**\nКуратор: {interaction.user.mention}"
    processed_embed = mark_request_processed_embed(processed_embed, status)
    processed_embed.color = discord.Color.green()
    await interaction.message.edit(embed=processed_embed, view=None)
    await interaction.followup.send("Заявка принята." if not note else "Заявка принята с оговоркой.", ephemeral=True)


class SphereCautionModal(Modal, title="Принять с оговоркой"):
    note = TextInput(label="Ошибки и замечания", style=discord.TextStyle.paragraph, required=True)
    ...
    async def on_submit(self, interaction: Interaction):
        await interaction.response.defer(ephemeral=True)
        await approve_sphere_request(interaction, self.request_id, str(self.note).strip())
...
class SphereReviewView(View):
    ...
    @discord.ui.button(label="✅ Принять", style=ButtonStyle.success, custom_id="sphere:review:approve")
    async def approve(self, interaction: Interaction, button: Button):
        await interaction.response.defer(ephemeral=True)
        await approve_sphere_request(interaction, self.request_id)

    @discord.ui.button(label="⚠️ Принять с оговоркой", style=ButtonStyle.primary, custom_id="sphere:review:approve_note")
    async def approve_with_note(self, interaction: Interaction, button: Button):
        await interaction.response.send_modal(SphereCautionModal(self.request_id))

    @discord.ui.button(label="❌ Отклонить", style=ButtonStyle.danger, custom_id="sphere:review:reject")
    async def reject(self, interaction: Interaction, button: Button):
...
class SphereLevelSetModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        try:
            ...
            return
        added_text = ", ".join(role.mention for role in roles_added) or "—"
        removed_text = ", ".join(role.mention for role in roles_removed) or "—"
        await interaction.response.send_message(
            embed=Embed(
                ...
                description=(
                    ...
                    f"🏛️ Уровень экономики: **{economy_level or 'не подходит ни под один уровень'}**\n"
                    f"➕ Выданы роли: {added_text}\n"
                    f"➖ Сняты роли: {removed_text}"
                ),
                color=0x00FF00,
            ),
            ephemeral=True,
...
class VerdictReviewSelect(Select):
    def __init__(self, req_id: str):
        ...
        options = [
            ...
            SelectOption(label="Автоизменение репутации", value="auto_rep", emoji="⭐"),
            SelectOption(label="Изменить уровень счастья", value="happy", emoji="🙂"),
            SelectOption(label="Автоизменение счастья", value="auto_happy", emoji="🙂"),
            SelectOption(label="Изменить науку", value="science", emoji="🔬"),
            SelectOption(label="Отношение ДРУГИХ к игроку", value="relation_in", emoji="🌐"),
            SelectOption(label="Отношение ИГРОКА к другим", value="relation_out", emoji="🌐"),
            SelectOption(label="Сменить уровень сферы", value="sphere_level", emoji="🧩"),
            SelectOption(label="Выдать предмет(ы)", value="item_add", emoji="🎁"),
            SelectOption(label="Изъять предмет(ы)", value="item_sub", emoji="📦"),
            SelectOption(label="Выдать население", value="population_add", emoji="👥"),
            SelectOption(label="Изъять население", value="population_sub", emoji="👥"),
            SelectOption(label="Выдать армию", value="army_add", emoji="🪖"),
            SelectOption(label="Изъять армию", value="army_sub", emoji="🪖"),
...
class VerdictResultView(View):
    def __init__(self, req_id: str):
        ...
        button = Button(
            ...
            custom_id=f"verdict:result:open:{self.req_id}",
        )
...
class PartnershipReviewView(View):
    ...
    async def approve(self, interaction: Interaction, button: Button):
        ...
        increment_admin_counter(interaction.user.id, "partnerships_done")
        save_partnership_data()
...
@bot.command(name="пиарменеджерроль")
@admin_or_command_access()
async def пиарменеджерроль(ctx, role: discord.Role):
    partnership_data["manager_role_id"] = role.id
    save_partnership_data()
    await ctx.send(embed=Embed(title="✅ Роль PR-менеджера установлена", description=f"Роль: {role.mention}", color=0x00FF00))


@bot.command(name="пиарпартнеркиканал")
@admin_or_command_access()
async def пиарпартнеркиканал(ctx, channel: discord.TextChannel):
    partnership_data["manager_post_channel"] = channel.id
    save_partnership_data()
    await ctx.send(embed=Embed(title="✅ Канал PR-партнерств установлен", description=f"Канал: {channel.mention}", color=0x00FF00))
...
class InvestmentActionModal(Modal):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        increment_admin_counter(interaction.user.id, "investments_accepted")
...
def ensure_company_country_presence_struct(company: dict):
    ...
    return company["country_presence"]
...
def relation_subject_key(raw: str) -> str:
    ...
    country_name = resolve_country_name(text)
    if country_name:
        active_season = get_active_registration_season()
        country_to_user, _ = get_occupied_country_map(active_season or None)
        owner_id = country_to_user.get(country_name)
        if owner_id:
            return str(owner_id)
        return country_name
...
def get_relation_value(source: str, target: str) -> int:
    source_key = str(source or "").strip()
    target_key = str(target or "").strip()
    if not source_key or not target_key:
        return 0
    relation_map = relations_data.setdefault("relations", {})
    values = relation_map.get(source_key, {})
    if not isinstance(values, dict):
        return 0
    try:
        return int(values.get(target_key, 0) or 0)
    except (TypeError, ValueError):
        return 0


def set_relation_value(source: str, target: str, value: int) -> int:
    source_key = str(source or "").strip()
    target_key = str(target or "").strip()
    ...
    try:
        ...
        new_value = 0
    relation_map = relations_data.setdefault("relations", {})
    values = relation_map.setdefault(source_key, {})
    if not isinstance(values, dict):
        values = {}
        relation_map[source_key] = values
    if new_value == 0:
        values.pop(target_key, None)
        if not values:
            relation_map.pop(source_key, None)
    else:
        values[target_key] = new_value
    return new_value


def get_country_display(subject: str) -> str:
    key = str(subject or "").strip()
    if not key:
        return "—"
    if key.isdigit():
        country_name = get_country_name_by_user_id(key)
        return country_name or f"<@{key}>"
    return str(relations_data.setdefault("pending_names", {}).get(key) or key)


def snapshot_relations_for_subject(subject: str) -> dict:
    key = str(subject)
    outgoing = make_json_snapshot(relations_data.setdefault("relations", {}).get(key, {}))
    incoming = {
        str(src): int(vals.get(key, 0) or 0)
        for src, vals in relations_data.setdefault("relations", {}).items()
        if isinstance(vals, dict) and key in vals
    }
    changes = [
        make_json_snapshot(change)
        for change in relations_data.setdefault("changes", [])
        if str((change or {}).get("source")) == key or str((change or {}).get("target")) == key
    ]
    return {"outgoing": outgoing, "incoming": incoming, "changes": changes}


def relation_snapshot_has_data(snapshot: dict) -> bool:
    return bool(
        isinstance(snapshot, dict)
        and (snapshot.get("outgoing") or snapshot.get("incoming") or snapshot.get("changes"))
    )


def remove_relations_for_subject(subject: str) -> bool:
    key = str(subject)
    changed = False
    if key in relations_data.setdefault("relations", {}):
        relations_data["relations"].pop(key, None)
        changed = True
    for vals in relations_data.setdefault("relations", {}).values():
        if isinstance(vals, dict) and key in vals:
            vals.pop(key, None)
            changed = True
    old_len = len(relations_data.setdefault("changes", []))
    relations_data["changes"] = [
        change for change in relations_data.setdefault("changes", [])
        if str((change or {}).get("source")) != key and str((change or {}).get("target")) != key
    ]
    return changed or len(relations_data["changes"]) != old_len


def restore_relations_for_subject(new_subject: str, snapshot: dict, old_subject: str | None = None):
    if not isinstance(snapshot, dict):
        return
    new_key = str(new_subject)
    old_key = str(old_subject or snapshot.get("source_user_id") or new_key)
    remove_relations_for_subject(new_key)
    outgoing = snapshot.get("outgoing", {}) if isinstance(snapshot.get("outgoing"), dict) else {}
    for target, value in outgoing.items():
        mapped_target = new_key if str(target) == old_key else str(target)
        set_relation_value(new_key, mapped_target, int(value or 0))
    incoming = snapshot.get("incoming", {}) if isinstance(snapshot.get("incoming"), dict) else {}
    for source, value in incoming.items():
        mapped_source = new_key if str(source) == old_key else str(source)
        set_relation_value(mapped_source, new_key, int(value or 0))
    for change in snapshot.get("changes", []) if isinstance(snapshot.get("changes"), list) else []:
        if not isinstance(change, dict):
            continue
        restored = make_json_snapshot(change)
        if str(restored.get("source")) == old_key:
            restored["source"] = new_key
        if str(restored.get("target")) == old_key:
            restored["target"] = new_key
        relations_data.setdefault("changes", []).append(restored)
...
def effective_company_share(company: dict, user_id: str, country_name: str | None = None) -> float:
    ...
    if str(user_id) == owner_id:
        return 1.0
    if not country_name:
        country_name = get_country_name_by_user_id(str(user_id))
    if not country_name:
        return 0.0
...
class CompanyCountryRejectModal(Modal):
    def __init__(self, req_id: str):
        self.req_id = str(req_id)
        super().__init__(title="Причина отказа страны", timeout=300)
        self.reason = TextInput(label="Причина", required=True, style=discord.TextStyle.paragraph, max_length=1000)
        self.add_item(self.reason)
...
class CompanyCountryDecisionView(View):
    def __init__(self, req_id: str):
        super().__init__(timeout=None)
        self.req_id = str(req_id)

    @discord.ui.button(label="ℹ️ Посмотреть информацию о компании", style=ButtonStyle.secondary, custom_id="company:country_info")
    async def info(self, interaction: Interaction, button: Button):
        req = companies_data.get("requests", {}).get(self.req_id)
        if not req:
            await interaction.response.send_message("❌ Заявка не найдена.", ephemeral=True)
            return
        ...
        if not company:
            ...
            return
        em = build_company_embed(company, 1, 1)
        em.add_field(name="Стоимость входа", value=str(req.get("entry_fee", "0")), inline=False)
        await interaction.response.send_message(embed=em, ephemeral=True)

    @discord.ui.button(label="✅ Согласиться", style=ButtonStyle.success, custom_id="company:country_accept")
    async def accept(self, interaction: Interaction, button: Button):
        req = companies_data.get("requests", {}).get(self.req_id)
        if not req:
            await interaction.response.send_message("❌ Заявка не найдена.", ephemeral=True)
            return
        if str(req.get("status")) != "pending_country":
            await interaction.response.send_message("❌ Эта заявка уже обработана.", ephemeral=True)
            return
        target_owner_id = str(req.get("target_country_owner_id") or "")
        if target_owner_id != str(interaction.user.id):
            await interaction.response.send_message("❌ Только владелец целевой страны может принять.", ephemeral=True)
            return
        company = companies_data.get("companies", {}).get(str(req.get("company_id")))
        if not company:
            await interaction.response.send_message("❌ Компания не найдена.", ephemeral=True)
    ...
    async def reject(self, interaction: Interaction, button: Button):
        ...
        if target_owner_id != str(interaction.user.id):
            await interaction.response.send_message("❌ Нет доступа.", ephemeral=True)
            return
        await interaction.response.send_modal(CompanyCountryRejectModal(self.req_id))


class CompanyEntryFeeModal(Modal):
    def __init__(self, req_id: str):
        self.req_id = str(req_id)
        super().__init__(title="Стоимость входа", timeout=300)
        self.fee = TextInput(label="Стоимость входа", required=True, max_length=100)
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        if not req:
            await interaction.response.send_message("❌ Заявка не найдена.", ephemeral=True)
            return
        req["entry_fee"] = str(self.fee.value).strip()
        req["status_text"] = f"📝 Добавлена стоимость входа: {self.fee.value}"
        save_companies_data()
        await interaction.response.send_message("✅ Стоимость входа сохранена.", ephemeral=True)


class CompanyForeignEntryRequestModal(Modal):
    def __init__(self):
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        req = {
            ...
            "target_country_owner_id": str(raw),
            "status": "pending_moderation",
            "status_text": "⏳ На рассмотрении модерации",
            "payload": {
                "Ссылка на пост": str(self.post_url.value).strip(),
                "Название компании": str(company.get("name", "")),
                "Кому": f"<@{raw}>",
            },
            "created_at": int(time.time()),
        }
        ...
        ch_id = companies_data.get("entry_requests_channel") or companies_data.get("requests_channel")
        ch = interaction.guild.get_channel(int(ch_id)) if ch_id else None
        if ch:
            msg = await ch.send(content=company_ping_roles_line(interaction.guild) or None, embed=build_company_request_embed(req), view=CompanyReviewView(str(req_id)))
            req["request_channel_id"] = msg.channel.id
            req["request_message_id"] = msg.id
            save_companies_data()
...
class CompanyReviewView(View):
    ...
    async def approve_request(self, interaction: Interaction):
        ...
        if req_type == "create":
            ...
            target_owner_id = str(req.get("target_country_owner_id") or "")
            target_country = str(req.get("target_country") or "")
            if not target_owner_id or not target_country:
    ...
    async def callback(self, interaction: Interaction):
        ...
        if choice == "entry_fee":
            if str(req.get("type")) != "foreign_entry":
                await interaction.response.send_message("❌ Это действие доступно только для заявок входа в страну.", ephemeral=True)
                return
            await interaction.response.send_modal(CompanyEntryFeeModal(view.req_id))
            return
        if choice == "approve":
...
class CompanyActionsSelect(Select):
    def __init__(self, *, include_owner_actions: bool = True):
        options = [
            SelectOption(label="Купить компанию", value="buy", emoji="🛒"),
        ]
        if include_owner_actions:
            options = [
                ...
                *options,
                ...
                SelectOption(label="Подать заявку на вход в страну", value="foreign_entry", emoji="🌍"),
            ]
    ...
    async def callback(self, interaction: Interaction):
        ...
        if v == "foreign_entry":
            await interaction.response.send_modal(CompanyForeignEntryRequestModal())
            return
...
class CompaniesMenuView(View):
    def __init__(self, pages: list[Embed], author_id: int, *, show_actions: bool = True):
        super().__init__(timeout=300)
        self.pages = pages
        self.author_id = author_id
        self.index = 0
        if show_actions:
            self.add_item(CompanyActionsSelect(include_owner_actions=show_actions))
...
async def show_companies_menu(ctx, member: discord.Member, viewer_id: int | None = None):
    ...
    if not companies:
        ...
        await ctx.send(embed=em, view=CompaniesMenuView([], viewer_id, show_actions=(viewer_id == member.id)))
        return

    pages = [build_company_embed(c, i + 1, len(companies)) for i, c in enumerate(companies)]
    await ctx.send(embed=pages[0], view=CompaniesMenuView(pages, viewer_id, show_actions=(viewer_id == member.id)))
...
@bot.command(name="заявкивходакомпаний")
@admin_or_command_access()
async def заявкивходакомпаний(ctx, channel: discord.TextChannel):
    companies_data["entry_requests_channel"] = channel.id
    save_companies_data()
    await ctx.send(embed=Embed(title="✅ Канал заявок входа компаний установлен", description=f"Канал: {channel.mention}", color=0x00FF00))


def find_rating_vote_by_message(message_id: int | str):
    for target_id, votes in ratings_data.setdefault("votes", {}).items():
        if not isinstance(votes, list):
            continue
        for vote in votes:
            if str(vote.get("message_id")) == str(message_id):
                return str(target_id), vote
    return None, None


class RatingPublishedActionSelect(Select):
    def __init__(self):
        super().__init__(
            placeholder="Действия с вашей оценкой",
            min_values=1,
            max_values=1,
            options=[
                SelectOption(label="Отредактировать оценку и комментарий", value="edit", emoji="✏️"),
                SelectOption(label="Удалить оценку", value="delete", emoji="🗑️"),
            ],
            custom_id="ratings:published_actions",
        )

    async def callback(self, interaction: Interaction):
        target_id, vote = find_rating_vote_by_message(interaction.message.id)
        if not vote:
            await interaction.response.send_message("❌ Оценка не найдена в базе.", ephemeral=True)
            return
        if str(vote.get("from")) != str(interaction.user.id):
            await interaction.response.send_message("❌ Управлять этой оценкой может только её публикатор.", ephemeral=True)
            return
        if self.values[0] == "edit":
            await interaction.response.send_modal(RatingModal(target_id, vote))
            return
        votes = ratings_data.setdefault("votes", {}).get(str(target_id), [])
        ratings_data["votes"][str(target_id)] = [v for v in votes if str(v.get("id")) != str(vote.get("id"))]
        save_ratings_data()
        try:
            ...
            pass
        if not interaction.response.is_done():
            await interaction.response.send_message("✅ Оценка удалена.", ephemeral=True)


class RatingPublishedActionView(View):
    def __init__(self):
        super().__init__(timeout=None)
        self.add_item(RatingPublishedActionSelect())


class RatingModal(Modal):
    def __init__(self, target_id: str, existing_vote: dict | None = None):
        self.target_id = str(target_id)
        self.existing_vote_id = str((existing_vote or {}).get("id") or "")
        super().__init__(title="Оценка администрации", timeout=300)
        self.role_input = TextInput(
            label="Назначение участника", required=True, max_length=120, default=str((existing_vote or {}).get("role_text") or "")
        )
        self.score_input = TextInput(label="Оценка (1-10)", required=True, max_length=2, default=str((existing_vote or {}).get("score") or ""))
        self.comment_input = TextInput(
            label="Комментарий",
            required=True,
            style=discord.TextStyle.paragraph,
            max_length=1000,
            default=str((existing_vote or {}).get("comment") or ""),
        )
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        payload = {
            "id": self.existing_vote_id or str(ratings_data.setdefault("next_vote_id", 1)),
            "from": voter_id,
            "score": score,
        ...
        if existing_vote:
            payload["channel_id"] = existing_vote.get("channel_id")
            payload["message_id"] = existing_vote.get("message_id")
            existing_vote.update(payload)
            stored_vote = existing_vote
        else:
        ...
        if channel:
            ...
            try:
                if existing_vote and stored_vote.get("message_id"):
                    msg = await channel.fetch_message(int(stored_vote.get("message_id")))
                    await msg.edit(content=target.mention, embed=embed, view=RatingPublishedActionView())
                else:
                    msg = await channel.send(content=target.mention, embed=embed, view=RatingPublishedActionView())
                    stored_vote["channel_id"] = msg.channel.id
                    stored_vote["message_id"] = msg.id
                    save_ratings_data()
            except Exception:
                pass
        ...
        try:
            await target.send(
                embed=Embed(
                    title="⭐ Вам оставили оценку",
                    description=(
                        f"**От:** {interaction.user.mention}\n"
                        f"**Оценка:** {score}/10\n"
                        f"**Назначение:** {str(self.role_input.value)}\n"
                        f"**Комментарий:** {str(self.comment_input.value)}"
                    ),
                    color=0x3498DB,
                )
            )
...
async def мойстат(ctx):
    ...
    if votes:
        ...
        avg_text = f"{avg:.2f}/10 ({len(votes)} оценок)"

    lines = [f"**Средняя оценка:** {avg_text}"]
    for key, label in ADMIN_COUNTER_LABELS.items():
        lines.append(f"**{label}:** {fmt_num(int(counters.get(key, 0)))}")

    class MyStatsRatingSelect(Select):
        def __init__(self):
            options = []
            for idx, vote in enumerate(votes[:25], start=1):
                voter = ctx.guild.get_member(int(vote.get("from", 0))) if str(vote.get("from", "")).isdigit() else None
                voter_name = voter.display_name if voter else f"ID {vote.get('from')}"
                options.append(SelectOption(label=f"{idx}) {voter_name} — {vote.get('score', 0)}/10", value=str(idx - 1), description=str(vote.get("comment") or "Без комментария")[:100]))
            super().__init__(placeholder="Посмотреть оценку и комментарий", options=options or [SelectOption(label="Оценок нет", value="none")])
        ...
        async def callback(self, interaction: Interaction):
            ...
            if self.values[0] == "none":
                await interaction.response.send_message("Оценок пока нет.", ephemeral=True)
                return
            vote = votes[int(self.values[0])]
            voter = ctx.guild.get_member(int(vote.get("from", 0))) if str(vote.get("from", "")).isdigit() else None
            voter_label = voter.mention if voter else f"<@{vote.get('from')}>"
            await interaction.response.send_message(embed=Embed(title="⭐ Оценка администратора", description=(f"**От:** {voter_label}\n**Оценка:** {vote.get('score', 0)}/10\n**Назначение:** {vote.get('role_text', '—')}\n\n**Комментарий:**\n{vote.get('comment', '—')}"), color=0x3498DB), ephemeral=True)

    view = View(timeout=180)
    view.add_item(MyStatsRatingSelect())
    await ctx.send(embed=Embed(title="📊 Мой стат", description="\n".join(lines), color=0x3498DB), view=view)
    ...
    if not collected_roles and not freeze_lines:
        ...
        await ctx.send(
            embed=Embed(
                ...
                description=(
                    f"{ctx.author.mention}, у вас нет ролей с ручным доходом для `!коллект`.\n\n"
                    "Получите роль с доходом или дождитесь, пока администрация настроит её для вас."
                ),
                color=0xFF0000,
            )
        )
...
class TopView(View):
    ...
    def _dataset(self):
        ...
        if self.mode == "happiness":
            users = player_state.get("users", {})
            return (
                sorted(
                    (
                        (uid, int((users.get(uid) or {}).get("happiness", 0)))
                        for uid in users.keys() if is_registered_country_user(uid)
                    ),
                    key=lambda x: x[1],
                    reverse=True,
...
class AdminTopModeSelect(Select):
    def __init__(self, owner: "TopView"):
        self.owner = owner
        options = [SelectOption(label="Всего действий", value="admin_total", emoji="🛡️")]
...
async def auto_role_income_loop():
    ...
    for guild in bot.guilds:
        ...
        # Пассивные изменения счастья/репутации из вердиктов
        state_changed = False
        for user_id, st in player_state.setdefault("users", {}).items():
            ...
            for entry in list(entries):
                ...
                if cooldown <= 0:
                    continue
                if "last_run" not in entry:
                    entry["last_run"] = now_ts - cooldown
                    state_changed = True
                last_run = int(entry.get("last_run", now_ts))
                if now_ts - last_run < cooldown:
                    continue
                ...
                delta = int(entry.get("amount", 0)) * cycles
                st[stat] = max(0, min(100, current + delta))
                entry["last_run"] = last_run + cycles * cooldown
                state_changed = True
...
async def создатьпредмет(ctx):
    ...
    FIELD_LABELS = {
        ...
        "wipe_delete": "Удалять при вайпе",
        "save_to_library": "Сохранять в библиотеку",
        "item_kind": "Сценарий",
        "tech_type": "Тип техники",
        "tech_rating": "Оценка техники",
    }
    ...
    def build_embed():
        ...
        if draft.get("item_kind") == "tech":
            ...
            e.add_field(name="Оценка", value=f"{int(draft.get('tech_rating', 1))}/100", inline=True)
    ...
    class EditFieldModal(Modal):
        def __init__(self, field_name: str):
            ...
            defaults = {
                ...
                "tech_rating": str(draft.get("tech_rating", 1)),
            }
            ...
            labels = {
                ...
                "wipe_delete": "Удалять при вайпе? (да/нет)",
                "save_to_library": "Сохранять в библиотеку? (да/нет)",
                "item_kind": "Сценарий: предмет/техника",
                "tech_type": "Тип техники",
                "tech_rating": "Оценка техники (1-100)",
            }
        ...
        async def on_submit(self, interaction: Interaction):
            ...
            try:
                if self.field_name == "key":
                    ...
                    draft["tech_type"] = raw
                    base = int(items_data["tech_types"][raw].get("min_price", 0))
                    draft["price"] = int(base * (1 + int(draft.get("tech_rating", 1)) / 100))
                elif self.field_name == "tech_rating":
                    rating = max(1, min(100, int(raw)))
                ...
                return
            await interaction.response.edit_message(embed=build_embed(), view=CreateItemView())
    ...
    async def apply_tech_type(interaction: Interaction, tech_type_name: str):
        ...
        base = int(items_data.setdefault("tech_types", {})[tech_type_name].get("min_price", 0))
        draft["price"] = int(base * (1 + int(draft.get("tech_rating", 1)) / 100))
        await interaction.response.edit_message(embed=build_embed(), view=CreateItemView())
        ...
        async def callback(self, interaction: Interaction):
            ...
            if selected == "save":
                ...
                item_payload = {
                    ...
                    "tech_type": draft.get("tech_type", ""),
                    "tech_rating": int(draft.get("tech_rating", 1)),
                }
    ...
    await ctx.send(embed=Embed(title="🛠️ Создание товара", description="Сначала выберите сценарий: обычный предмет или техника.", color=0x3498DB), view=KindView())
    ...
    async def select_callback(interaction: Interaction):
        ...
        def build_page(page_index: int):
            ...
            for item in page_items:
                ...
                desc += format_shop_item_description(item, count) + "\n"
            e = Embed(
                title=f"🛒 {category_name}",
...
async def предметинфо(ctx, *, item_query: str):
    ...
    if item.get("item_kind") == "tech":
        embed.add_field(name="Тип техники", value=item.get("tech_type") or "—", inline=True)
        embed.add_field(name="Оценка техники", value=f"{int(item.get('tech_rating', 1))}/100", inline=True)
    ...
    embed.add_field(
        name="Удалять при !вайп",
        value=bool_to_yes_no(item.get("wipe_delete", False)),
        inline=True,
    )
    embed.add_field(
        name="Сохранять в библиотеку техники",
        value=bool_to_yes_no(item.get("save_to_library", False)),
        inline=True,
    )
    await ctx.send(embed=embed)
...
async def рег(ctx, member: discord.Member, country: str, year: str):
    ...
    async def finalize_registration():
        ...
        use_archived_stats = False
        if isinstance(archived_snapshot, dict):
            class ArchivedStatsView(View):
                def __init__(self):
                    super().__init__(timeout=180)
                    self.result = None
                ...
                async def interaction_check(self, interaction: Interaction):
                    if interaction.user.id == ctx.author.id or interaction.user.guild_permissions.administrator:
                        return True
                    await interaction.response.send_message(
                        "❌ Выбрать перенос статистики может только инициатор или администратор.",
                        ephemeral=True,
                    )
                    return False

                @discord.ui.button(label="✅ Да, перенести", style=ButtonStyle.success)
                async def yes(self, interaction: Interaction, button: Button):
                    self.result = True
                    await interaction.response.edit_message(
                        embed=Embed(title="✅ Старая статистика будет перенесена", color=0x00FF00),
                        view=None,
                    )
                    self.stop()

                @discord.ui.button(label="❌ Нет, начать с нуля", style=ButtonStyle.secondary)
                async def no(self, interaction: Interaction, button: Button):
                    self.result = False
                    await interaction.response.edit_message(
                        embed=Embed(title="🆕 Игрок начнёт с нуля", color=0xAAAAAA),
                        view=None,
                    )
                    self.stop()

            view = ArchivedStatsView()
            await ctx.send(
                embed=Embed(
                    title="📦 Найдена старая статистика страны",
                    description=(
                        f"Для **{resolved_country}** в сезоне **{year_str}** найдены сохранённые деньги, инвентарь, сферы, отношения, стрики и другие данные прошлого игрока.\n"
                        "Перенести эту статистику на нового игрока?"
                    ),
                    color=0x3498DB,
                ),
                view=view,
            ...
            await view.wait()
            use_archived_stats = bool(view.result)
...
def build_relations_embed(subject: str) -> Embed:
    outgoing = relations_data.setdefault("relations", {}).get(str(subject), {})
    incoming = [(src, vals.get(str(subject), 0)) for src, vals in relations_data.setdefault("relations", {}).items() if isinstance(vals, dict) and str(subject) in vals]
    e = Embed(title=f"🌐 Отношения: {get_country_display(subject)}", color=0x3498DB)
    out_lines = [f"• {get_country_display(t)}: **{int(v):+d}**" for t, v in sorted(outgoing.items(), key=lambda x: get_country_display(x[0]).lower())]
    in_lines = [f"• {get_country_display(src)}: **{int(v):+d}**" for src, v in sorted(incoming, key=lambda x: get_country_display(x[0]).lower())]
    e.add_field(name="Он к другим", value="\n".join(out_lines)[:1024] or "—", inline=False)
    e.add_field(name="Другие к нему", value="\n".join(in_lines)[:1024] or "—", inline=False)
    return e
...
async def профиль(ctx, member: discord.Member = None):
    ...
    embed.description = "\n".join(profile_lines)
    embed.set_footer(text=f"Актуально на RP-год: {get_current_rp_year_display()}")
